...
# 1. 페이지 기본 설정
st.set_page_config(
    page_title="scienter의 상담소 - 청소년 MBTI & 진로 탐색",
    page_icon="🌊",
    layout="centered"
)

# 2. 시원한 여름 테마 및 청소년 친화적 커스텀 CSS
st.markdown("""
    <style>
...
# 3. 데이터 정의 (MBTI 추천 직업 & 청소년 통계 데이터)
mbti_data = {
    "INTJ": [
...
# 한국 청소년 MBTI 전체 분포 통계 (어세스타 표본 기준)
teen_mbti_overall = {
...
st.markdown("""
    <div class="main-header">
        <h1>🌊 scienter의 상담소</h1>
        <p>청소년을 위한 MBTI 분석실 & 맞춤 진로 상담소</p>
    </div>
""", unsafe_allow_html=True)

# 5. 페이지 탭 구성 (1번: 청소년 MBTI 분포 / 2번: MBTI 직업 추천)
tab1, tab2 = st.tabs(["📊 1. 한국 청소년 MBTI 분포", "🎯 2. MBTI 맞춤 직업 추천"])

# ==================== 1번 탭 : 청소년 MBTI 분포 ====================
with tab1:
    st.subheader("🇰🇷 한국 중·고등학생 MBTI 유형 분포")
    st.write("한국 청소년 통계 데이터에 기반한 성격 유형 분포와 남녀 성비 차이입니다.")

    # (1) 전체 TOP MBTI 차트
    st.markdown("#### 🏆 청소년에게 가장 많은 MBTI 비율 (TOP 5)")
    ...
    # 전체 16가지 유형 비율표
    with st.expander("👀 16가지 전체 MBTI 비율 펼쳐보기"):
    ...
    # (2) 남녀 성별 MBTI 선호 지표 분석
    st.subheader("👫 청소년 남녀 성비에 따른 지표 차이")
    st.write("청소년 시기 남학생과 여학생은 특히 **T(사고형) vs F(감정형)**와 **S(감각) vs N(직관)**에서 뚜렷한 특징을 보입니다.")

    st.markdown("##### 1️⃣ T(사고) vs F(감정) 비율 비교")
    st.markdown("""
        <div class="stat-card">
            <div class="stat-label">
                <span>👦 남학생 (T: 52% | F: 48%)</span>
            </div>
            <div class="bar-bg"><div class="bar-fill-m" style="width: 52%;"></div></div>
        </div>
        <div class="stat-card">
            <div class="stat-label">
                <span>👧 여학생 (T: 28% | F: 72%)</span>
            </div>
            <div class="bar-bg"><div class="bar-fill-f" style="width: 28%;"></div></div>
        </div>
    """, unsafe_allow_html=True)
    st.caption("📌 **분석**: 여학생은 감정(F) 선호 비율이 70% 이상으로 공감과 관계 중심적인 성향이 높게 나타납니다.")

    st.markdown("##### 2️⃣ 청소년기 S(감각) vs N(직관) 특성")
    col_m, col_f = st.columns(2)
    with col_m:
        st.info("**청소년 남학생 주요 특징**\n- **ESTJ, ISTJ, ENFP** 비율이 높음\n- 논리와 사실 기반 문제 해결을 선호하는 경향")
    with col_f:
        st.success("**청소년 여학생 주요 특징**\n- **ENFP, INFP, ISFP** 비율이 높음\n- 상상력, 감성, 표현력이 풍부한 유형이 상대적으로 높음")

    st.markdown("""
        <div class="counselor-box">
            💡 <b>scienter 선생님의 분석 노트!</b><br>
            "청소년기에는 성인에 비해 **E(외향)**와 **N(직관)**, **F(감정)**의 비율이 더 높게 나타나는 경향이 있어요.<br>
            친구들과의 관계와 풍부한 감수성을 발전시키는 시기이기 때문이랍니다! 🌱"
        </div>
    """, unsafe_allow_html=True)
